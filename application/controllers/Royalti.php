...
class Royalti extends CI_Controller {
    ...
    public function dt_show()
    {
        ...
        if ( $valid ) {
            ...
            $total = $this->m_royalti->count_data($search, $acc_level, $id_perusahaan);
            $result = $this->m_royalti->load_data($page, $perpage, $order_by, $search, $acc_level, $id_perusahaan);
            $data = array(
                "draw" => 0,
                "recordsTotal" => $total,
    ...
    public function get_one()
    {
        ...
        if ( $valid ) {
            $status = True;
            $message = "Showing data";
            $data = $this->m_royalti->get_one($id);
        
            $log_activity = array(
                "app_id" => $app_id,
                "route" => "royalti/get_one",
                "created" => date("Y-m-d H:i:s"),
                "action" => "r",
                "activity" => json_encode($data)
            );
    ...
    public function insert()
    {
        ...
        $valid = $this->m_app_auth->is_valid($app_id, $app_token);
        if ( $valid ) {
            $exist = $this->m_royalti->is_exist($id);
            if ( ! $exist ) {
                $data = array(
                ...
                $exe = $this->m_royalti->insert($data);
                if ( $exe ) {
                    $status = True;
                    $message = "Insert Royalti Success";
                    $data["id"] = $this->m_royalti->get_last_id();
                }
            } else {
    ...
    public function update()
    {
        ...
        $valid = $this->m_app_auth->is_valid($app_id, $app_token);
        if ( $valid ) {
            $prev_data = $this->m_royalti->get_one($id);
            $data = array(
                "thn_prajamrek" => $this->input->post("thn_prajamrek"),
                "rupiah_prajamrek" => $this->input->post("rupiah_prajamrek"),
                "dollar_prajamrek" => $this->input->post("dollar_prajamrek"),
            ...
            $exe = $this->m_royalti->update($data, $id);
            if ( $exe ) {
                $status = True;
                $message = "Update Royalti Success";
            }
        
            $log_activity = array(
                "app_id" => $app_id,
                "route" => "royalti/update",
                "created" => date("Y-m-d H:i:s"),
                "action" => "u",
    ...
    public function delete()
    {
        ...
        if ( $valid ) {
            $data = $this->m_royalti->get_one($id);
            $exe = $this->m_royalti->delete($id);
            if ( $exe ) {
                $status = True;
                $message = "Delete Royalti Success";
            }
        
            $log_activity = array(
                "app_id" => $app_id,
                "route" => "royalti/delete",
                "created" => date("Y-m-d H:i:s"),
                "action" => "d",
                "activity" => json_encode($data)
            );
