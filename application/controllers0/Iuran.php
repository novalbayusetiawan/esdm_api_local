...
class Iuran extends CI_Controller {
    ...
    public function dt_show()
    {
        ...
        if ( $valid ) {
            ...
            $total = $this->m_iuran->count_data($search, $acc_level, $id_perusahaan);
            $result = $this->m_iuran->load_data($page, $perpage, $order_by, $search, $acc_level, $id_perusahaan);
            $data = array(
                "draw" => 0,
                "recordsTotal" => $total,
    ...
    public function insert()
    {
        ...
        $valid = $this->m_app_auth->is_valid($app_id, $app_token);
        if ( $valid ) {
            $exist = $this->m_iuran->is_exist($id);
            if ( ! $exist ) {
                $data = array(
                ...
                $exe = $this->m_iuran->insert($data);
                if ( $exe ) {
                    $status = True;
                    $message = "Insert Iuran Success";
                    $data["id"] = $this->m_iuran->get_last_id();
                }
            } else {
    ...
    public function update()
    {
        ...
        $valid = $this->m_app_auth->is_valid($app_id, $app_token);
        if ( $valid ) {
            $prev_data = $this->m_iuran->get_one($id);
            $data = array(
                "thn_prajamrek" => $this->input->post("thn_prajamrek"),
                "rupiah_prajamrek" => $this->input->post("rupiah_prajamrek"),
                "dollar_prajamrek" => $this->input->post("dollar_prajamrek"),
            ...
            $exe = $this->m_iuran->update($data, $id);
            if ( $exe ) {
                $status = True;
                $message = "Update Iuran Success";
            }
    ...
    public function delete()
    {
        ...
        if ( $valid ) {
            $data = $this->m_iuran->get_one($id);
            $exe = $this->m_iuran->delete($id);
            if ( $exe ) {
                $status = True;
                $message = "Delete Iuran Success";
            }
